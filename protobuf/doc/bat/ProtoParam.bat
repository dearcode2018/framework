@ rem ----- 信息 -----
@ rem @filename ProtoParam.bat
@ rem @version 1.0
@ rem @author qye.zheng
@ rem @description 参数配置
...
:: 该参数通常不用修改
set outputSrcPath=src/main/java


:: 指定每次要生成的 proto 文件的位置，例如 doc/proto/UserMessage.proto

:: 通过文本文件 path.txt 取出 proto 文件路径值
for /f %%i in (path.txt) do (
	set protoPath=%%i
)

::set protoPath=doc/proto/UserMessage.proto
